...
1. **Initialize directories:** Before exporting, create the pet-specific directories under `art/source/pets/<pet>/`, `art/export/pets/<pet>/`, and `docs/pets/<pet>/` along with per-state subfolders (for example `art/source/pets/<pet>/<state>/`, `art/export/pets/<pet>/<state>/`, and, when state-specific evidence is planned, `docs/pets/<pet>/<state>/`). Use shell helpers such as `mkdir -p art/source/pets/<pet>`, `mkdir -p art/source/pets/<pet>/ambient_idle`, `mkdir -p art/export/pets/<pet>/ambient_idle`, and `mkdir -p docs/pets/<pet>` so the folder tree exists ahead of automation. Drop a placeholder `README.md` or `.keep` file into any newly created directory to ensure it remains tracked in version control.
...
5. **Commit expectations:** Stage the generated directories plus optimized resources under `app/src/main/res/raw/` and `app/src/main/res/drawable/`. Include the export logs emitted under `art/export/pets/<pet>/export.log` so reviewers can trace tooling versions.
...
8. **Documentation update:** Ensure `docs/pets/<pet>/animation.md` exists (create it if necessary, e.g., `touch docs/pets/<pet>/animation.md`) and append an entry summarizing export settings, optimization parameters, and integration commit hash before completing the checklist item.
