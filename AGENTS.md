...
- **Execution principles:**
 ...
 6. Debug quickly with print statements and share the captured output when looping.
  ...
  8. When providing terminal commands for the user to copy and paste, supply complete ready-to-run commands with all values filled; if any user-specific info is required, ask for it first and only respond with the finalized command.
...
1. **[x] Prompt:** _"Audit the repository guardrails. Inspect `README.md`, `CODEOWNERS`, `CONTRIBUTING.md`, `LICENSE`, `.gitignore`, `.editorconfig`, and `.gitattributes`; record any needed adjustments in `docs/project/decision_log.md`; capture current branch protection status and document how to request updates."_ _(Completed: guardrails reviewed, findings logged, branch-protection screenshot captured (2025-12-01 01:09 UTC))_
   ...
   - **Steps:**
     ...
     - [x] Capture the current branch-protection settings (rules, required checks, enforcement status) for the default branch and obtain a screenshot.
...
16. **[ ] Prompt:** _"Enable Play App Signing in the Google Play Console and capture evidence."_ _(claimed by @agent, 2025-12-01 13:13 UTC)_ _(Blocked: Play Console access/SSO credentials unavailable in this environment; cannot enable or capture evidence (2025-12-01 13:14 UTC))_
    ...
    - **Local credential cue:** Current upload keystore alias = `cosmobond-upload`; key password = store password (same value). If password forgotten, hint: “It is your favorite password that starts with an S and ends with a 7.”
