...
## 2025-12-01 - Task 15: Nightly Macrobenchmark workflow
- Added a `generateBaselineProfile` alias task in `baselineprofile/build.gradle.kts` (BaselineProfile task group) that wraps `connectedNonMinifiedReleaseAndroidTest` and copies the first `baseline-prof.txt` found under `app/build` into `app/src/main/baseline-prof.txt` for packaging; the task fails fast if no profile is produced, leaving repo state uncommitted.
- Created scheduled workflow `.github/workflows/baselineprofile-nightly.yml` (07:00 UTC cron + manual dispatch) that boots a Wear OS API 34 emulator via `ReactiveCircus/android-emulator-runner@v2`, runs `./gradlew --stacktrace :baselineprofile:generateBaselineProfile`, and uploads `artifacts/baselineprofile/` containing the copied profile plus `baselineprofile/build/outputs/macrobenchmark` perf metrics.
