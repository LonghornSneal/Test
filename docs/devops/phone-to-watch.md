...
1. On your phone (Termux or any shell with GitHub CLI installed), run:
   ```bash
   ...
   ```
   - The helper script pulls the newest successful `Build installable artifacts` run and downloads available artifacts into `~/Downloads/cosmobond-build/`.
...
2. Pick your install method (ADB sideload or Play upload) from the sections below and point it at the downloaded APK/AAB.
3. After installing, open **CosmoBond** on the watch, save once, and confirm the face updates without reopening the picker (see validation checklist at the bottom).
